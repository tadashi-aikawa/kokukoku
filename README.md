...
- **連続時間表示**: 連続作業時間はフッター中央の和ろうそくで表す。アラート閾値の最大値までを燃焼時間として溶けていき、燃え尽きると熾火と立ち上る煙になる。初期待機・休憩中・リセット直後は火を消した新品の状態
...
| `G` | 末尾へ移動 |
| `]` | 最初のプロジェクト行へ移動 |
| `Enter` | 選択中のアクションを実行（プロジェクト行=計測切替、予定行=カレンダーの詳細ページを開いてパネルを閉じる、「他◯件/畳む」=展開・畳む） |
| `Escape` | パネルを閉じる |
| `e` | 選択中プロジェクトの累積時間を編集 |
...
休憩に入ると連続作業時間はリセットされ、和ろうそくは火の消えた新品に戻ります。初期待機・休憩中に連続作業時間を編集した場合、その値は次のプロジェクト開始時に引き継がれます。

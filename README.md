...
endNotificationLeadMinutes = 5 # 進行中予定の終了の何分前に通知するか (省略時は通知しない)
...
カレンダー連携は macOS のカレンダー (EventKit) を経由します。システム設定 > インターネットアカウントに Google アカウントを追加しておいてください。Google カレンダー側の変更が反映されるまで数分 (実測 3〜4 分) かかるため、`notificationLeadMinutes` / `endNotificationLeadMinutes` を 5 分より短くすると直前の予定変更を拾えない可能性があります。

予定の開始 `notificationLeadMinutes` 分前に、パネルが自動表示されて該当予定が強調されます。`endNotificationLeadMinutes` を設定すると、進行中の予定の終了の指定分前にも同じ通知が出ます (MTG を締めて次の行動へ移る合図。省略時は出ません)。

自分で作業時間を確保するために入れている予定 (「確保」など) は `ignoreTitles` に予定名を並べると KOKUKOKU 側で無かったことになり、一覧にも開始前通知にも出なくなります。判定は予定名の完全一致です (前後の空白は無視します)。

パネルで `C` を押すと、終了済みを含む本日の予定を次の形式でクリップボードへコピーします。終日予定・辞退済み予定・`ignoreTitles` で除外した予定は含みません。
...
| `C` | 終了済みを含む本日の予定を箇条書きテキストとしてクリップボードにコピー（終日・辞退済み・`ignoreTitles` 対象は除外） |
...
予定の詳細ポップオーバーには、カレンダーの詳細ページを開くボタンとオンライン会議の参加ボタンが状況に応じて並びます。ボタンを選択した状態で `Enter` を押すとそのボタンを実行します。ボタンを実行するとブラウザが開き、パネルも閉じます（パネル固定中はポップオーバーだけ閉じ、パネルは残ります）。

...
public enum PanelLayout {
    ...
    /// 大きさは枠の数字ではなく**目を引く大玉の直径**で語ること: 玉簪は玉しか目に入らないため、
    /// 外接の数字ほど大きく見えない(軸が全長だった頃は枠40で玉7.9px = 花簪だった頃の
    /// 花12.8pxの6割で、実機で控えめすぎた)。
    /// 軸を薄墨へ溶かして外接の縦が縮んだ(55→41)ぶん、同じ枠でも玉が育つ。
    /// 枠48 × 玉半径4.4 で玉10.3px(2026-07-27 タダシ選定)。
    /// いちど枠48 × 玉半径5.4(玉12.6px)で実機に出したが大きすぎたため、枠は据え置いて
    /// 玉のほうを実物寄りに落とした。枠を戻さないのは、玉と小玉の距離を稼ぐため
    public static let pinButtonHeight = 48.0
